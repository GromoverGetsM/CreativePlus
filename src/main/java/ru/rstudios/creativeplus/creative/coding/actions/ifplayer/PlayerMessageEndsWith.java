...
public class PlayerMessageEndsWith extends ActionIf {
    ...
    public PlayerMessageEndsWith(Starter starter, String name, Inventory inventory, List<Action> actions) {
    ...
    public boolean conditionExpression(GameEvent event, List<Entity> selection) {
        if (!(event instanceof ChatEvent)) {
            for (Player player : event.getPlot().getPlotOnlineList()) {
                ...
                player.sendMessage("§bCreative+ §8» §fВызвано событие с несовместимым условием 'Сообщение заканчивается на'");
            }
            return false;
        }

        for (ItemStack item : this.getTexts()) {
            if (((ChatEvent) event).getMessage().endsWith(CodingHandleUtils.parseText(item))) return true;
        }
        return false;
    }
