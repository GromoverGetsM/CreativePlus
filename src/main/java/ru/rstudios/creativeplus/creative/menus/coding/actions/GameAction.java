...
public class GameAction extends AbstractSelectCategoryMenu {
    ...
    public GameAction (String name, int rows) {
        this(name, rows, null);
    }

    public GameAction (String name, int rows, HashMap<Integer, ItemStack> items) {
    ...
    public static class CodeUtilCategory extends AbstractCategoryMenu {
        ...
        public CodeUtilCategory() {
            this("Утилиты кода");
        }
        public CodeUtilCategory (String name) {
            this(name, 1);
        }

        public CodeUtilCategory (String name, int rows) {
            this(name, rows, null);
        }

        public CodeUtilCategory (String name, int rows, HashMap<Integer, ItemStack> items) {
            super(name, rows, items);
            this.name = name;
            this.rows = rows;
            this.items = items;
        ...
        @Override
        public @NotNull Inventory getInventory() {
            File inventory = new File(plugin.getDataFolder() + File.separator + "menus" + File.separator + "coding" + File.separator + "actions" + File.separator + "gameaction" + File.separator + this.getClass().getSimpleName() + ".yml");
            Inventory i;
